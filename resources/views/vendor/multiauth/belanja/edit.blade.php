...
<div class="container">
    ...
    @include('multiauth::message')
                    <form action="{{route('belanja.update',$belanja->id)}}" method="post">
                        @csrf
                        @method('patch')
                        <div class="form-group">
                            <label for="kode_belanja">Kode Belanja</label>
                            <input type="text" placeholder="Kode Belanja" name="kode_belanja" value="{{$belanja->kode_belanja}}" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label for="nama_belanja">Nama Belanja</label>
                            <input type="text" placeholder="Nama Belanja" name="nama_belanja" value="{{$belanja->nama_belanja}}" class="form-control" required>
                        </div>
                        <div class="form-group">
                        <label for="belanja">Kegiatan</label>
                        <select class="form-control" name="kegiatan">
                          @foreach ($kegiatan as $keg)
                            <option value="{{$keg->id}}"@if ($keg->id == $belanja->kegiatan_id)
                              selected
